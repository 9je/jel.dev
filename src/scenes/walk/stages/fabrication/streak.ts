...
import baked from '../../../../content/streak.json';
 ...
 *
 * The board draws the figure baked in at build time and then repaints itself if the server's live
 * copy comes back with a different one, so the wall is right on a day with no deploy behind it. See
 * `streak-feed.ts` for where that copy comes from and `scripts/streak.mjs` for how it is read.
 *
...
/** The two faces the board is printed with, and the canvas each was printed on so a new figure can
 *  be printed over it. The colour map, and the emissive map that carries only the things that light
 *  up, which is the count and the days with something on them. Painted by one routine so the glow
 *  can never drift from the print. */
type Face = { c: HTMLCanvasElement; tex: THREE.CanvasTexture; glow: boolean };

function face(data: StreakData, glow: boolean): Face {
  const [c, ctx] = canvas(W, H);
  paint(ctx, data, glow);
  return { c, tex: own(c), glow };
}

function paint(ctx: CanvasRenderingContext2D, data: StreakData, glow: boolean): void {
  const r = rng(17);
  const digits = String(data.current).padStart(3, '0');
  ...
  for (let col = 0; col < 12; col++) {
    for (let rowN = 0; rowN < 7; rowN++) {
      const level = Number(data.recent[col * 7 + rowN] ?? 0);
  ...
  if (!glow) {
    ...
    ctx.fillText(`READ ${data.generated}`, x0, H - 34);
    ctx.textAlign = 'right'; ctx.fillText(`GITHUB.COM/${data.login.toUpperCase()}`, W - 34, H - 34);
  }
}
...
export function streakBoard(): THREE.Group {
  ...
  const data: StreakData = readStreak(baked) ?? (baked as StreakData);
  const map = face(data, false), glow = face(data, true);
  const mat = new THREE.MeshStandardMaterial({
    map: map.tex, emissive: 0xffffff, emissiveMap: glow.tex, emissiveIntensity: 0.9, roughness: 0.42, metalness: 0.05,
  });
  const panel = new THREE.Mesh(new THREE.PlaneGeometry(w, h), mat);
  panel.name = 'face'; g.add(panel);

  // Somebody changed the number. Repaint both faces in place: the material, the geometry and the
  // draw call are the ones already in the room, so this costs two canvas fills and two uploads.
  onStreak((next) => {
